...
public:
	...
	/** Return the text contained in the part.
	  ...
	  */
	virtual const std::shared_ptr<const contentHandler> getText() const = 0;

	/** Set the text contained in the part.
	  *
	  * @param text text of the part
	  */
	virtual void setText(std::shared_ptr<contentHandler> text) = 0;
	...
	/** Generate the text part(s) into the specified message.
	  ...
	  */
	virtual void generateIn(std::shared_ptr<bodyPart> message, std::shared_ptr<bodyPart> parent) const = 0;
	...
	/** Parse the text part(s) from the specified message.
	  ...
	  */
	virtual void parse(std::shared_ptr<const bodyPart> message, std::shared_ptr<const bodyPart> parent, std::shared_ptr<const bodyPart> textPart) = 0;
};
