...
public:
	...
	sendableMDNInfos(const std::shared_ptr<const message> msg, const mailbox& mbox);
	sendableMDNInfos(const sendableMDNInfos& other);

	sendableMDNInfos& operator=(const sendableMDNInfos& other);

	const std::shared_ptr<const message> getMessage() const;
...
private:
	...
	std::shared_ptr<const message> m_msg;
	mailbox m_mailbox;
};
