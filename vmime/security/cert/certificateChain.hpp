...
public:
	...
	/** Construct a new certificateChain object given an ordered list
	  ...
	  */
	certificateChain(const std::vector <std::shared_ptr<certificate> >& certs);
	...
	/** Return the certificate at the specified position. 0 is the
	  ...
	  */
	std::shared_ptr<certificate> getAt(const unsigned int index);

protected:

	std::vector <std::shared_ptr<certificate> > m_certs;
};
