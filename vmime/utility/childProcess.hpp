...
public:
	...
	/** Return a wrapper to the child process standard input.
	  ...
	  */
	virtual std::shared_ptr<utility::outputStream> getStdIn() = 0;

	/** Return a wrapper to the child process standard output.
	  *
	  * @return input stream wrapper for child's stdout
	  */
	virtual std::shared_ptr<utility::inputStream> getStdOut() = 0;
...
public:
	...
	/** Create a new child process.
	  ...
	  */
	virtual std::shared_ptr<childProcess> create(const utility::file::path& path) const = 0;
};
