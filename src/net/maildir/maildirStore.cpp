...
maildirStore::maildirStore(std::shared_ptr<session> sess, std::shared_ptr<security::authenticator> auth)
...
std::shared_ptr<folder> maildirStore::getRootFolder()
{
	if (!isConnected())
		throw exceptions::illegal_state("Not connected");

	return vmime::std::make_shared<maildirFolder>(folder::path(),
		thisRef().dynamicCast <maildirStore>());
}


std::shared_ptr<folder> maildirStore::getDefaultFolder()
{
	if (!isConnected())
		throw exceptions::illegal_state("Not connected");

	return vmime::std::make_shared<maildirFolder>(folder::path::component("inbox"),
		thisRef().dynamicCast <maildirStore>());
}


std::shared_ptr<folder> maildirStore::getFolder(const folder::path& path)
{
	if (!isConnected())
		throw exceptions::illegal_state("Not connected");

	return vmime::std::make_shared<maildirFolder>(path,
		thisRef().dynamicCast <maildirStore>());
}
...
void maildirStore::connect()
{
	...
	std::shared_ptr<utility::file> rootDir = fsf->create(m_fsPath);
...
std::shared_ptr<maildirFormat> maildirStore::getFormat()
{
	return m_format;
}


std::shared_ptr<const maildirFormat> maildirStore::getFormat() const
{
	return m_format;
}
