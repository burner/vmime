...
kmailMaildirFormat::kmailMaildirFormat(std::shared_ptr<context> ctx)
	: maildirFormat(ctx)
{
}
	...
	(std::vector <folder::path>& list, const folder::path& root, const bool recursive) const
{
	...
	std::shared_ptr<utility::file> rootDir = fsf->create(folderPathToFileSystemPath(root,
...
void kmailMaildirFormat::renameFolder(const folder::path& oldPath, const folder::path& newPath)
{
	...
	std::shared_ptr<utility::file> contDir = fsf->create
