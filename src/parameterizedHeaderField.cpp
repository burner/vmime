	...
	const string::size_type end, string::size_type* newPosition)
{
	...
	if (p < pend)
	{
		     ...
		     it != params.end() ; ++it)
		{
			...
			std::shared_ptr<parameter> param = vmime::std::make_shared<parameter>((*it).first);
	...
	const string::size_type curLinePos, string::size_type* newLinePos) const
{
	...
	// Parameters
	for (std::vector <std::shared_ptr<parameter> >::const_iterator
...
bool parameterizedHeaderField::hasParameter(const string& paramName) const
{
	...
	std::vector <std::shared_ptr<parameter> >::const_iterator pos = m_params.begin();
	const std::vector <std::shared_ptr<parameter> >::const_iterator end = m_params.end();
...
std::shared_ptr<parameter> parameterizedHeaderField::findParameter(const string& paramName) const
{
	const string name = utility::stringUtils::toLower(paramName);
...
std::shared_ptr<parameter> parameterizedHeaderField::getParameter(const string& paramName)
{
	const string name = utility::stringUtils::toLower(paramName);
	...
	if (pos == end)
	{
		std::shared_ptr<parameter> param = vmime::std::make_shared<parameter>(paramName);
...
void parameterizedHeaderField::appendParameter(std::shared_ptr<parameter> param)
{
	m_params.push_back(param);
}


void parameterizedHeaderField::insertParameterBefore(std::shared_ptr<parameter> beforeParam, std::shared_ptr<parameter> param)
{
	const std::vector <std::shared_ptr<parameter> >::iterator it = std::find
...
void parameterizedHeaderField::insertParameterBefore(const int pos, std::shared_ptr<parameter> param)
{
	m_params.insert(m_params.begin() + pos, param);
}


void parameterizedHeaderField::insertParameterAfter(std::shared_ptr<parameter> afterParam, std::shared_ptr<parameter> param)
{
	const std::vector <std::shared_ptr<parameter> >::iterator it = std::find
...
void parameterizedHeaderField::insertParameterAfter(const int pos, std::shared_ptr<parameter> param)
{
	m_params.insert(m_params.begin() + pos + 1, param);
}


void parameterizedHeaderField::removeParameter(std::shared_ptr<parameter> param)
{
	const std::vector <std::shared_ptr<parameter> >::iterator it = std::find
...
{
	const std::vector <std::shared_ptr<parameter> >::iterator it = m_params.begin() + pos;
...
const std::shared_ptr<parameter> parameterizedHeaderField::getParameterAt(const int pos)
{
	return (m_params[pos]);
}


const std::shared_ptr<const parameter> parameterizedHeaderField::getParameterAt(const int pos) const
{
	return (m_params[pos]);
}


const std::vector <std::shared_ptr<const parameter> > parameterizedHeaderField::getParameterList() const
{
	std::vector <std::shared_ptr<const parameter> > list;

	list.reserve(m_params.size());

	for (std::vector <std::shared_ptr<parameter> >::const_iterator it = m_params.begin() ;
	     it != m_params.end() ; ++it)
	{
		list.push_back(*it);
	}
...
const std::vector <std::shared_ptr<parameter> > parameterizedHeaderField::getParameterList()
{
	return (m_params);
}


const std::vector <std::shared_ptr<component> > parameterizedHeaderField::getChildComponents()
{
	std::vector <std::shared_ptr<component> > list = headerField::getChildComponents();

	for (std::vector <std::shared_ptr<parameter> >::iterator it = m_params.begin() ;
	     it != m_params.end() ; ++it)
	{
		list.push_back(*it);
	}
