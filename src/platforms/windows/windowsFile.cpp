...
{
	return vmime::factory<windowsFile>::create(path);
}
...
std::shared_ptr<vmime::utility::file> windowsFile::getParent() const
{
	if (m_path.isEmpty())
		...
		return vmime::factory<windowsFile>::create(m_path.getParent());
}
...
{
	return vmime::factory<windowsFileWriter>::create(m_path, m_nativePath);
}

std::shared_ptr<vmime::utility::fileReader> windowsFile::getFileReader()
{
	return vmime::factory<windowsFileReader>::create(m_path, m_nativePath);
}

std::shared_ptr<vmime::utility::fileIterator> windowsFile::getFiles() const
{
	return vmime::factory<windowsFileIterator>::create(m_path, m_nativePath);
}
...
{
	std::shared_ptr<vmime::utility::file> pFile =
		vmime::factory<windowsFile>::create
...
std::shared_ptr<vmime::utility::inputStream> windowsFileReader::getInputStream()
{
	...
	if (hFile == INVALID_HANDLE_VALUE)
		windowsFileSystemFactory::reportError(m_path, GetLastError());
	return vmime::factory<windowsFileReaderInputStream>::create(m_path, hFile);
}
...
std::shared_ptr<vmime::utility::outputStream> windowsFileWriter::getOutputStream()
{
	...
	if (hFile == INVALID_HANDLE_VALUE)
		windowsFileSystemFactory::reportError(m_path, GetLastError());
	return vmime::factory<windowsFileWriterOutputStream>::create(m_path, hFile);
}
