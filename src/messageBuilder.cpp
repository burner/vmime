...
std::shared_ptr<message> messageBuilder::construct() const
{
	...
	if (!m_attach.empty() && m_textPart->getPartCount() > 1)
	{
		...
		std::shared_ptr<bodyPart> subPart = vmime::std::make_shared<bodyPart>();
	...
	if (!m_attach.empty())
	{
		for (std::vector <std::shared_ptr<attachment> >::const_iterator a = m_attach.begin() ;
		     a != m_attach.end() ; ++a)
		{
			(*a)->generateIn(msg);
		}
	...
	if (msg->getBody()->getPartCount() == 1)
	{
		...
		for (std::vector <std::shared_ptr<const headerField> >::const_iterator it = fields.begin() ;
		     it != fields.end() ; ++it)
		{
...
void messageBuilder::attach(std::shared_ptr<attachment> attach)
{
	appendAttachment(attach);
}


void messageBuilder::appendAttachment(std::shared_ptr<attachment> attach)
{
...
{
	std::shared_ptr<textPart> part = NULL;
...
std::shared_ptr<textPart> messageBuilder::getTextPart()
{
	return (m_textPart);
}
...
const std::shared_ptr<const attachment> messageBuilder::getAttachmentAt(const int pos) const
{
	return (m_attach[pos]);
}


std::shared_ptr<attachment> messageBuilder::getAttachmentAt(const int pos)
{
	return (m_attach[pos]);
}
...
const std::vector <std::shared_ptr<const attachment> > messageBuilder::getAttachmentList() const
{
	std::vector <std::shared_ptr<const attachment> > res;

	res.reserve(m_attach.size());

	for (std::vector <std::shared_ptr<attachment> >::const_iterator it = m_attach.begin() ;
	     it != m_attach.end() ; ++it)
	{
		res.push_back(*it);
	}
...
const std::vector <std::shared_ptr<attachment> > messageBuilder::getAttachmentList()
{
	return (m_attach);
}
