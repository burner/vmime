...
parameter::parameter(const string& name)
	: m_name(name), m_value(vmime::factory<word>::create())
{
}


parameter::parameter(const string& name, const word& value)
	: m_name(name), m_value(vmime::factory<word>::create(value))
{
}
...
{
	std::shared_ptr<parameter> p = vmime::factory<parameter>::create(m_name);
	p->copyFrom(*this);
