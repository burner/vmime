	...
	: object()
{
	for (std::list <std::shared_ptr<property> >::const_iterator it = set.m_props.begin() ; it != set.m_props.end() ; ++it)
		m_props.push_back(vmime::std::make_shared<property>(**it));
}
...
{
	std::list <std::shared_ptr<property> >::iterator it = std::find_if
...
void propertySet::parse(const string& props)
{
	...
	for ( ; pos != end ; )
	{
		...
		if (pos != end)
		{
			...
			m_props.push_back(vmime::std::make_shared<property>(option, value));
		}
	}
}


std::shared_ptr<propertySet::property> propertySet::find(const string& name) const
{
	std::list <std::shared_ptr<property> >::const_iterator it = std::find_if
		(m_props.begin(), m_props.end(), propFinder(name));
...
std::shared_ptr<propertySet::property> propertySet::findOrCreate(const string& name)
{
	std::list <std::shared_ptr<property> >::const_iterator it = std::find_if
		(m_props.begin(), m_props.end(), propFinder(name));
	...
	{
		std::shared_ptr<property> prop = vmime::std::make_shared<property>(name, "");
...
const std::vector <std::shared_ptr<const propertySet::property> > propertySet::getPropertyList() const
{
	std::vector <std::shared_ptr<const property> > res;
...
const std::vector <std::shared_ptr<propertySet::property> > propertySet::getPropertyList()
{
