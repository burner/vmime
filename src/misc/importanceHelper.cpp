...
void importanceHelper::resetImportance(std::shared_ptr<message> msg)
{
	resetImportanceHeader(msg->getHeader());
}


void importanceHelper::resetImportanceHeader(std::shared_ptr<header> hdr)
{
	try
	{
		std::shared_ptr<headerField> fld = hdr->findField("X-Priority");
		hdr->removeField(fld);
	}
	...
	try
	{
		std::shared_ptr<headerField> fld = hdr->findField("Importance");
		hdr->removeField(fld);
	}
	catch (exceptions::no_such_field)
	{
...
importanceHelper::Importance importanceHelper::getImportance(std::shared_ptr<const message> msg)
{
	return getImportanceHeader(msg->getHeader());
}


importanceHelper::Importance importanceHelper::getImportanceHeader(std::shared_ptr<const header> hdr)
{
...
void importanceHelper::setImportance(std::shared_ptr<message> msg, const Importance i)
{
	setImportanceHeader(msg->getHeader(), i);
}


void importanceHelper::setImportanceHeader(std::shared_ptr<header> hdr, const Importance i)
{
	// "X-Priority:" Field
	std::shared_ptr<headerField> fld = hdr->getField("X-Priority");
