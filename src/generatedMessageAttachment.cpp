...
generatedMessageAttachment::generatedMessageAttachment(std::shared_ptr<const bodyPart> part)
	: m_bpa(vmime::std::make_shared<bodyPartAttachment>(part))
{
}
...
const std::shared_ptr<const contentHandler> generatedMessageAttachment::getData() const
{
...
std::shared_ptr<const object> generatedMessageAttachment::getPart() const
{
	return m_bpa->getPart();
}


std::shared_ptr<const header> generatedMessageAttachment::getHeader() const
{
	return m_bpa->getHeader();
}


std::shared_ptr<message> generatedMessageAttachment::getMessage() const
{
	if (m_msg == NULL)
	{
		// Extract data
		...
		// Parse message
		m_msg = vmime::std::make_shared<message>();
		m_msg->parse(oss.str());
	}
...
void generatedMessageAttachment::generateIn(std::shared_ptr<bodyPart> /* parent */) const
{
